# Cohort 1 launch checklist

Target start: **Monday, September 8, 2026**

## −16 weeks (May 2026)

- [ ] EVP approves program concept ([PROPOSAL.md](../../PROPOSAL.md))
- [ ] EVP names hiring partner pipeline owner
- [ ] General Counsel begins [Program Agreement](../templates/legal/program-agreement.md) review
- [ ] Finance approves $245k reserve ([financial-model.md](../../business/financial-model.md))
- [ ] Registrar creates continuing-ed certificate shell

## −12 weeks (June 15, 2026)

- [ ] Applications open ([admissions.md](../../operations/admissions.md))
- [ ] Landing page live with cost of attendance disclosure
- [ ] Admissions take-home repo published ([admissions-take-home](../admissions-take-home/))
- [ ] Founder begins first-5 partner outreach ([hiring-partners.md](../../partnerships/hiring-partners.md))
- [ ] Marketing cleared by GC (word "guaranteed")

## −8 weeks (July 2026)

- [ ] Hult corporate relations begins partner slots 4–5
- [ ] Program Agreement finalized and signable
- [ ] Expectations Acknowledgment wired into enrollment flow
- [ ] Insurance bound ([legal-risk.md](../../institutional/legal-risk.md))

## −4 weeks (August 2026)

- [ ] Applications close Aug 15
- [ ] Admissions decisions Aug 29
- [ ] Cohort ≥ 20 confirmed
- [ ] Partner agreements: ≥ 3 signed (stretch: 5)
- [ ] GitHub org `hult-cohort-fall26-boston` created
- [ ] Cohort project template pushed to org

## −1 week (Sep 1, 2026)

- [ ] Pre-Cohort Setup email sent ([tooling-setup.md](../../curriculum/onboarding/tooling-setup.md))
- [ ] Discord bootstrap server created
- [ ] Tuition billing ready for Sep 8 charge

## Week 1 (Sep 8, 2026)

- [ ] Tuition charged; roster locked Sep 12 after refund window
- [ ] 100% tooling verification complete
- [ ] Week 2 Project 1 kickoff ready

## Platform (parallel track)

- [ ] Oct 15: Ludwitt/Hult JWT spec frozen
- [ ] Nov 1: Sandbox + production API live ([ludwitt-hult-platform.md](../../partnerships/ludwitt-hult-platform.md))
- [ ] Nov 3: First student app approved (smoke test)

## Week 16 (Dec 19, 2026)

- [ ] Showcase event ([showcase-event.md](../../partnerships/showcase-event.md))
- [ ] Pass/fail published Dec 19–20
- [ ] Placement window opens
