...
5. `npm run dev` → sign up, create project, create + assign task
6. Deployed to Vercel (`pm-joes9987.vercel.app`)
...
- Supabase env vars must be set in Vercel project settings for full auth/data (schema in repo; free-tier project limit reached during setup — use existing Supabase project or pause one to provision)
- No due dates, notifications, or GitHub issue linking yet
- No review/vote module (differentiator backlog for later projects)
- Email confirmation may need disabling in Supabase for frictionless reviewer access

## Agent usage summary

- **Research:** Project 1 requirements, submission format, cohort template conventions
- **Development:** Cursor Agent scaffolded Next.js app, Supabase schema + RLS, auth flows, project/task UI, Vercel deploy
- **QA:** Production URL smoke test, fresh-clone setup checklist documented in README
