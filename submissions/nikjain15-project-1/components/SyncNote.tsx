  ...
  if (outcome.kind === 'no_handle') {
    return (
      ...
      >
        ...
        <Link href="/connect" className="underline hover:text-red-200">
          Reconnect GitHub
        </Link>{' '}
        to fix it. The board below works either way.
      </p>
    );
  }
  ...
  if (outcome.kind === 'write_failed') {
    return (
      ...
      >
        ...
        {(outcome.created > 0 || outcome.moved > 0) && ' Some cards did land.'} Nothing is lost, and
        you can still add and move cards yourself.
      </p>
    );
  }
