...
export async function forgetShared(db: Firestore, handle: string): Promise<number> {
  ...
  // Also erase this person's agent-to-agent tasks. These live in a top-level collection keyed by
  // the handle field (not under the context doc), and each carries a free-text intent + payload —
  // so "be forgotten" must sweep them too, or the user's cross-app requests survive erasure.
  const tasks = await db.collection(BUS.tasks).where('handle', '==', contextKey(handle)).get();
...
export async function claimTasks(db: Firestore, toApp = APP, handle: string | null = null, limit = 10): Promise<AgentTask[]> {
  ...
  // pending task is always claimed first — makes claiming fair and starvation-free.
